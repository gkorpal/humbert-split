#!/usr/bin/env bash
#
# Rebuild the .txt datasets in this directory from their split pieces.
#
#   ./merge.sh            # merge everything (~3.8 GB of output)
#   ./merge.sh 1000bit    # only files whose name contains a substring
#
# The original files run 96-317 MB, over GitHub's 100 MB per-blob limit, so each
# one is stored here as a sequence of line-aligned pieces:
#
...
# script just does it for every file, in order, and verifies the result:
#
#   cat polz_1000bit_faa1c19f_100000.txt.part?? > polz_1000bit_faa1c19f_100000.txt
#
# Needs ~3.8 GB free. Safe to re-run: files already merged are skipped. The
